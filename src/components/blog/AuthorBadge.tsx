...
export default function AuthorBadge({ author, className }: AuthorBadgeProps) {
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      {imageUrl ? (
        <div className="relative w-10 h-10 rounded-full overflow-hidden border border-primary/15">
          <Image src={imageUrl} alt={author.name} fill sizes="40px" className="object-cover" />
        </div>
      ) : (
        <div className="w-10 h-10 rounded-full border border-primary/15 flex items-center justify-center text-[11px] uppercase tracking-widest text-foreground/70 bg-white">
